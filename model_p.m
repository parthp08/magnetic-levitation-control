...
Kd = 1400;
PID = pid(Kp,Ki,Kd);
cl_sys_1 = feedback(tf_1, PID); % closed loop

% impulse response
figure();
impulseplot(cl_sys_1);

% step response
opt = stepDataOptions('StepAmplitude',1);
figure();
stepplot(cl_sys_1, opt);
...
figure();
rlocusplot(cl_sys_1); % notice pole zero cancelation and one dominant pole pair in left half plane
